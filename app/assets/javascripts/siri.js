...
var cookie_not_handle_user_settings = true;
    ...
    $('.user-div .box .box-tool .box-tool2 > a').click(function(e) {
        if ($(this).data('action') == undefined) {
            return;
        }
        var action = $(this).data('action');
        ...
        switch (action) {
            case 'collapse':
                $(btn).children('i').addClass('anim-turn180');
                $(this).parents('.user-div .box').children('.user-div .box-content').slideToggle(500, function(){
                    if($(this).is(":hidden")) {
                        $(btn).children('i').attr('class', 'icon-chevron-down');
                    } else {
                        $(btn).children('i').attr('class', 'icon-chevron-up');
                ...
                break;
            case 'close':
                $(this).parents('.user-div .box').fadeOut(500, function(){
                    $(this).parent().remove();
                })
                break;
            case 'config':
